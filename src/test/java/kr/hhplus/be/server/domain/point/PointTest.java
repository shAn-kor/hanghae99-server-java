...
class PointTest {
    ...
    void createPoint_invalid_pointId() {
        UUID userId = UUID.randomUUID();
        assertThatThrownBy(() -> new Point( userId, 5000L))
    ...
    void createPoint_null_userId() {
        assertThatThrownBy(() -> new Point( null, 5000L))
                .isInstanceOf(IllegalArgumentException.class)
    ...
    void createPoint_null_balance() {
        UUID userId = UUID.randomUUID();
        assertThatThrownBy(() -> new Point( userId, null))
                .isInstanceOf(IllegalArgumentException.class)
    ...
    void createPoint_zeroOrNegative_balance() {
        UUID userId = UUID.randomUUID();
        assertThatThrownBy(() -> new Point( userId, 0L))
                .isInstanceOf(IllegalArgumentException.class)
                .hasMessage("balance must be greater than 0");
    }
    ...
    void checkPoint_true() {
        Point point = new Point( UUID.randomUUID(), 10000L);
    ...
    void checkPoint_false() {
        Point point = new Point( UUID.randomUUID(), 1000L);
