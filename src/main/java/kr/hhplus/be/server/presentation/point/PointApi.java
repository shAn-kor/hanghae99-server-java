...
public interface PointApi {
    ...
    @PostMapping("/charge")
    ResponseEntity.BodyBuilder charge(@Valid @RequestBody PointRequest pointRequest);

    @Operation(summary = "포인트 내역 조회", description = "특정 유저의 포인트 사용/충전 이력을 조회합니다.")
    @GetMapping("/history")
    ResponseEntity<List<PointHistoryResponse>> history(@Valid @RequestBody PointRequest pointRequest);
}